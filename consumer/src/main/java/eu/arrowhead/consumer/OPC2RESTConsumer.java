...
public class OPC2RESTConsumer {
    ...
    private ServiceRequestForm compileSRF() {
      ...
      SystemName can be an arbitrarily chosen name, which makes sense for the use case.
        ...
        metadata.put("identifier", "Triangle1");
        if (isSecure) {
      ...
      so the communication between consumer and provider can be facilitated.
         */
        ArrowheadService service = new ArrowheadService("Triangle1", Collections.singleton("json"), metadata);
    ...
    private void consumeService(String providerUrl) {
        // FIXME The line below is added due to encoding problems.
        providerUrl = providerUrl.replace("%3F", "?");
        //providerUrl += "11";
        System.out.println("\n\nGET request to service: " + providerUrl);
        Response response = Utility.sendRequest(providerUrl, "GET", null);
        System.out.println("Response from service: " + response.readEntity(String.class));
    }
