...
public class FullProviderMain extends ArrowheadClientMain {
  ...
  public enum ServiceRegistryActions {
    REGISTER, UNREGISTER
  }

  // NOTE!! The opcServerAddress should NOT include opc.tcp://
  private String opcServerAddress = "A9824.neteq.ltu.se:53530/OPCUA/SimulationServer";
  private int rootNodeNamespaceIndex = 5;
  private String rootNodeIdentifier = "85/0:Simulation";
  ...
  private FullProviderMain(String[] args) {
    Set<Class<?>> classes = new HashSet<>(Arrays.asList(RESTHandler.class));
    ...
    requestToServiceRegistry(ServiceRegistryActions.REGISTER);
    if (NEED_AUTH) {
  ...
  protected void shutdown() {
    requestToServiceRegistry(ServiceRegistryActions.UNREGISTER);
    if (server != null) {
      server.shutdownNow();
    }
  ...
  private void loadAndCompilePayloads(boolean fromFile, String nodeString) {
    String metadataString = nodeString;
    ...
    if (fromFile) {
      ...
      URI uri;
      String serviceUri = "opcReadwrite?serverAddress=" + opcServerAddress + "&ns=" + metadata.get("ns") + "&id=" + metadata.get("identifier") + "&value=";
      String serviceDef = metadata.get("displayName");
  ...
  private void requestToServiceRegistry(ServiceRegistryActions action) {
    ...
    try {
      // FIXME These things should be read from the config file!
      ...
      for(String nodeString:nodesBeneath) {
        loadAndCompilePayloads(false, nodeString);
